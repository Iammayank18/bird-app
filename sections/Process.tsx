...
const Process: React.FC = () => {
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="flex flex-col lg:flex-row items-start lg:items-end justify-between mb-12 sm:mb-16 gap-6">
        <div className="max-w-2xl">
          ...
          <h2 className="text-primary font-heading font-black text-3xl sm:text-4xl md:text-5xl tracking-tight">How We Deliver Excellence</h2>
        </div>
        <p className="text-slate-500 text-base sm:text-lg lg:max-w-xs">A seamless journey from your first call to a bird-free property.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-4 relative">
        ...
        <div className="lg:hidden absolute left-[1.75rem] top-8 w-0.5 h-[calc(100%-4rem)] bg-gray-100 -z-10"></div>

        {steps.map((step, idx) => (
          <div key={idx} className="group relative">
            <div className="mb-4 lg:mb-8 flex items-center justify-start">
              <div className="relative z-10 w-14 h-14 rounded-full bg-white border-4 border-background-light shadow-xl flex items-center justify-center text-primary group-hover:bg-primary group-hover:text-white transition-all duration-300">
                ...
                <div className="absolute -top-1 -right-1 bg-secondary text-white text-[10px] font-bold h-6 w-6 rounded-full flex items-center justify-center border-2 border-white">{step.num}</div>
              </div>
            </div>
            <div className="ml-16 lg:ml-0 p-5 sm:p-6 bg-gray-50 rounded-2xl border border-transparent group-hover:border-secondary/20 group-hover:bg-white transition-all duration-300">
